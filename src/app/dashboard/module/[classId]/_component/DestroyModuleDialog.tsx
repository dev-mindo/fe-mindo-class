...
export const DestroyModuleDialog = (props: Props) => {
  ...
  const destroyModule = async () => {
    const destroy: ApiResponse = await fetchApi(
      `/admin/module/${props.module.id}`,
      {
        method: 'DELETE'
      }
    );
    ...
    if (destroy) {
      if (destroy.statusCode === 200) {        
        toast.info(`Data ${props.module.title} berhasil dihapus`);
        setIsOpen(false);
    ...
    <AlertDialog open={isOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Apakah kamu ingin menghapus {props.module.title}?
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
